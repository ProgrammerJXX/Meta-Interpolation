...
class Dataset_Model94(BaseDataset):
    def __init__(self, config, img_paths, sampler, mode, dtype=torch.float32, add_gussian=False):
        ...
        Note:
            This method initializes various attributes of the Dataset_Model94 class,
            including configuration, image paths, seed, sampler, mode, and noise level.
        """
        super(Dataset_Model94, self).__init__() # 没有的话不继承BaseDataset的方法

        self.config = config

        if mode == 'train':
            self.img_paths = self.list_txt_files(img_paths)[:6000]  # 6000
            ...
            self.img_paths = self.list_txt_files(img_paths)[:2000]  # 2000
